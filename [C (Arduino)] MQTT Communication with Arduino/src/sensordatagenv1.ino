...
void onConnectionEstablished()
{
  ...
  client.publish("shelf1"," is online");
  Serial.print("shelf1 has reported to MQTT broker.");
}

// pseudo replacement for read_average() from scale hx711 function
...
void loop() 
{
  ...
  std::string s = String(temp);
  Serial.print("sensor1TOPIC",s);
  client.publish("sensor1TOPIC",s);
  client.loop();
  delay(5000);
}